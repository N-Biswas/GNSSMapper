...
public class SettingsFragment extends Fragment {
  ...
  /** User selected location to be recorded alongside measurements */
  ...
  /** User selection of location, initially set to be disabled */
  private boolean mUserLocationSetting = false;
  /** The reference ground truth location by user input. */
  private double[] mUserLocation = null;
  ...
  public View onCreateView(
    ...
    final Switch registerUserLocation = (Switch) view.findViewById(R.id.register_user_location);
    final TextView registerUserLocationLabel =
              (TextView) view.findViewById(R.id.register_user_location_label);
    //set the switch to OFF
    registerUserLocation.setChecked(false);
    registerUserLocationLabel.setText("Switch is OFF");
    registerUserLocation.setOnCheckedChangeListener(
              new OnCheckedChangeListener() {
                  @Override
                  public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                      if (isChecked) {
                          ...
                          LayoutInflater inflater =
                                  (LayoutInflater)
                                          getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                          View layout = inflater.inflate(R.layout.pop_up_window2,
                                  (ViewGroup) getActivity().findViewById(R.id.pop));
                          ...
                          params.dimAmount = 0.5f;
                          wm.updateViewLayout(container, params);
                          mUserLocationSetting = false;
                          // When the window is dismissed same as cancel
                          popupWindow.setOnDismissListener(
                                  new OnDismissListener() {
                                      @Override
                                      public void onDismiss() {
                                          if (mUserLocationSetting) {
                                              registerUserLocationLabel.setText("Switch is ON");
                                          } else {
                                              registerUserLocation.setChecked(false);
                                          }
                                      }
                          ...
                          buttonOk.setOnClickListener(
                                  new OnClickListener() {
                                      ...
                                      public void onClick(View v) {
                                          ...
                                          mUserLocation =
                                                  new double[] {latitudeDegrees, longitudeDegrees, altitudeMeters};
                                          mUserLocationSetting = true;
                                          mGpsContainer.setUserLocation(mUserLocationSetting,mUserLocation);
                                          popupWindow.dismiss();
                                      }
                                  }
                          );
