...
public class UiLogger implements GnssListener {
  ...
  public void onGnssMeasurementsReceived(GnssMeasurementsEvent event,double[] userLocation) {
    ...
    builder.append(toStringLocation(userLocation));
    builder.append("]\n");

    logMeasurementEvent("onGnssMeasurementsReceived: " + builder.toString());
  }

  private String toStringLocation(double[] location){
    final String format = "   %-4s = %s\n";
    StringBuilder builder = new StringBuilder("UserIndicatedLocation: \n");
    if (location == null){
      builder.append("not set\n");
    } else {
    DecimalFormat numberFormat = new DecimalFormat("#0.000");
      builder.append(String.format(format, "Latitude", numberFormat.format(location[0])));
      builder.append(String.format(format, "Longitude", numberFormat.format(location[1])));
      builder.append(String.format(format, "Altitude", numberFormat.format(location[2])));
    }
