...
public class FileLogger implements GnssListener {
    ...
    synchronized (mFileLock) {
      ...
      try {
        ...
        currentFileWriter.write(
            "Raw,ElapsedRealtimeNanos,TimeNanos,LeapSecond,TimeUncertaintyNanos,FullBiasNanos,"
  ...
  @Override
  public void onGnssMeasurementsReceived(GnssMeasurementsEvent event, double[] userLocation) {
    synchronized (mFileLock) {
      if (mFileWriter == null) {
        return;
      }
      GnssClock gnssClock = event.getClock();
      long systemClockEstimate = SystemClock.elapsedRealtimeNanos();
      for (GnssMeasurement measurement : event.getMeasurements()) {
        try {
          writeGnssMeasurementToFile(gnssClock, systemClockEstimate, measurement, userLocation);
        } catch (IOException e) {
          logException(ERROR_WRITING_FILE, e);
        }
      }
  ...
  private void writeGnssMeasurementToFile(GnssClock clock, long systemClockEstimate, GnssMeasurement measurement, double[] userLocation)
      throws IOException {
    String clockStream =
        String.format(
            "Raw,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
            systemClockEstimate, //once upgraded to API 29 GNSSclock has an associated elapsedrealtimenanos.
            clock.getTimeNanos(),
    ...
    String locationStream =
              String.format(
                      "%s,%s,%s",
                      userLocation==null ? "":userLocation[0],
                      userLocation==null ? "":userLocation[1],
                      userLocation==null ? "":userLocation[2]);
